...
std::array<revmodel,2> reverb;
Trill trill;
std::array<CentroidDetection,4> cenDet;
...
unsigned int velumDigIn = switch1;
unsigned int wobbleDigIn = trigIn1;
...
void trillRead(void*)
{
	...
	{
		trill.readI2C();
		for(size_t n = 0; n < cenDet.size(); ++n)
		{
			size_t offset = n * (trill.getNumChannels() / cenDet.size());
			cenDet[n].process(trill.rawData.data() + offset);
			printf("%7.3f ", cenDet[n].compoundTouchSize());
		}
		printf("\n");
		usleep(8000);
	}
...
{
	trill.setup(1, Trill::CRAFT, 0x38);
	usleep(10000);
	trill.setPrescaler(3);
	usleep(10000);
	trill.setMode(Trill::DIFF);
	for(size_t n = 0; n < cenDet.size(); ++n)
		cenDet[n].setup(trill.getNumChannels() / cenDet.size(), 1, 1500);
	Bela_runAuxiliaryTask(trillRead);
...
#include <algorithm>
float mapAndConstrain(float in, float inMin, float inMax, float outMin, float outMax)
{
	in = map(in, inMin, inMax, outMin, outMax);
	if(outMax < outMin)
		std::swap(outMin, outMax);
	return std::min(outMax, (std::max(outMin, in)));
}
void render(BelaContext* context, void*)
{
	#if 0
	for(size_t n = 0; n < context->analogInChannels * context->analogFrames; ++n)
		((float*)context->analogIn)[n] = std::min(1, 3.f *((float*)context->analogIn)[n]);
	#endif
	...
	if(1)
	{
		...
		Tract.velumTarget =  velumInput ? 0.99 : 0.01 ;
		// echo velum to LED
	...
	for(size_t n = 0; n < reverb.size(); ++n)
	{
		reverb[n].setdamp(mapAndConstrain(cenDet[0].compoundTouchSize(), 0, 0.5, 0.2, 1));
		reverb[n].setroomsize(mapAndConstrain(cenDet[3].compoundTouchSize(), 0, 0.4, 0.2, 1));
		reverb[n].setwidth(0);
		int pin = 0 == n ? switch1 : switch2;
		bool freeze = digitalRead(context, 0, pin);
		pin = 0 == n ? trigIn3 : trigIn4;
		freeze |= digitalRead(context, 0, pin);
		reverb[n].setmode(freeze);
		reverb[n].setwet(freeze ? 1 : 0.4);
		reverb[n].setdry(freeze ? 0 : 1);
	}
	if(context->audioFramesElapsed > 1.f * context->audioSampleRate)
		for(unsigned int n = 0; n < context->audioFrames; ++n)
		{
			float tract = tractOutArray[n];
			std::array<float,2> out = {};
			for(size_t c = 0; c < reverb.size(); ++c)
				reverb[c].processmix(&tract, &tract, out.data() + c, out.data() + c, 1, 1);
